...
public class TreeImpl<E extends Comparable<? super E>> implements Tree<E> {
    ...
    public TreeImpl() {
        limitLevel = -1; // -1 means no limit
    }

    public TreeImpl(int limitLevel) {
    ...
    private NodeAndParent doFind(E value) {
        Node<E> current = root;
        Node<E> parent = null;
        int parentLevel = root == null ? 0 : 1;

        while (current != null) {
            if (current.getValue().equals(value)) {
                return new NodeAndParent(current, parent, parentLevel);
            }
        ...
        return new NodeAndParent(null, parent, parentLevel);
    }
    ...
    public boolean add(E value) {
        ...
        if (limitLevel > 0 && nodeAndParent.parentLevel == limitLevel) {
            return false;
        }
    ...
    public void traverse(TraversMode mode) {
        switch (mode) {
            case PRE_ORDER:
                preOrder(root); //прямой обход
                break;
            case IN_ORDER:
                inOrder(root); //центрированный обход
                break;
            case POST_ORDER:
                postOrder(root); //обратный порядок
                break;
            default:
